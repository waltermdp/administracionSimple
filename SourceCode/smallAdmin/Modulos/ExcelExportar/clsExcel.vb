   ...
   Public Class clsExcel
      ...
      Public Shared Function AddWorksheet(spreadsheet As DocumentFormat.OpenXml.Packaging.SpreadsheetDocument, name As String) As Boolean
         Try
            ...
            sheet = New DocumentFormat.OpenXml.Spreadsheet.Sheet With {
                  ...
                  .SheetId = CUInt(spreadsheet.WorkbookPart.Workbook.Sheets.Count() + 1),
                   .Name = name}
            sheets.Append(sheet)
            spreadsheet.WorkbookPart.Workbook.Save()
    ...
    Public Shared Function ColumnNameFromIndex(columnIndex As UInt32) As String
      Try
        ...
        While (columnIndex > 0)
          remainder = CUInt((columnIndex - 1) Mod 26)
          columnName = System.Convert.ToChar(65 + remainder).ToString() + columnName
          columnIndex = CUInt((columnIndex - remainder) / 26)
        End While
...
#End If
    ...
    Public Shared Function SetHiperlinkCellValue(spreadsheet As DocumentFormat.OpenXml.Packaging.SpreadsheetDocument, worksheet As DocumentFormat.OpenXml.Spreadsheet.Worksheet, columnIndex As UInt32, rowIndex As UInt32, stringValue As String, vlink As String, styleIndex As UInt32?, Optional save As Boolean = True) As Boolean
      Try
        ...
        hyper.Id = "UNIQUE" & rowIndex ' "UNIQUE" & rowIndex

        Hypers.Append(hyper)

        worksheet.WorksheetPart.AddHyperlinkRelationship(New System.Uri(vlink, System.UriKind.Absolute), True, hyper.Id)
      ...
      Try
        ...
        Else
          ...
          Dim prevRowQuery = From item In sheetData.Elements(Of DocumentFormat.OpenXml.Spreadsheet.Row)()
                             ...
                             Select item
          For counter As Int32 = CInt(rowIndex - 1) To 1 Step -1
            loopCounter = counter
        ...
        Else
          ...
          Dim prevCellQuery = From item In row.Elements(Of DocumentFormat.OpenXml.Spreadsheet.Cell)()
                              Where item.CellReference.Value = clsExcel.ColumnNameFromIndex(CUInt(loopCounter)) & rowIndex
          For counter As Int32 = CInt(columnIndex - 1) To 1 Step -1
            loopCounter = counter
        ...
        If Not (styleIndex Is Nothing) Then
          cell.StyleIndex = CUInt(styleIndex)
