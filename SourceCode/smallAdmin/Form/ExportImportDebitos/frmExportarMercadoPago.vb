...
Public Class frmExportarMercadoPago
  ...
  Private Sub frmExportarPatagonia_Shown(sender As Object, e As EventArgs) Handles Me.Shown
    Try
      m_Banco = New clsMercadoPago(m_TipoPago.GuidTipo)
      m_skip = True
  ...
  Private Sub RecargarValores()
    Try
      ...
      ClsInfoExportarMercadoPagoBindingSource.DataSource = m_Banco.m_RegistrosExportar
      ClsInfoExportarMercadoPagoBindingSource.ResetBindings(False)
  ...
  Private Function updateOpcionesFiltrado() As Boolean
    Try
      ...
      If rbAplicaMesesAnteriores.Checked Then
        m_Banco.ModoFiltrado = clsMercadoPago.E_FiltroMesesAnteriores.MESES_ANT_CON_FILTRO
      ElseIf rbMesesAnterioresSinFiltro.Checked Then
        m_Banco.ModoFiltrado = clsMercadoPago.E_FiltroMesesAnteriores.MESES_ANT_SIN_FILTRO
      Else
        m_Banco.ModoFiltrado = clsMercadoPago.E_FiltroMesesAnteriores.SOLO_MES_ACTUAL
  ...
  Private Sub dgvResumen_CellMouseUp(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvResumen.CellMouseUp
    Try
      If e.ColumnIndex = 0 Then
        Dim auxInfo As New clsInfoExportarMercadoPago
        auxInfo = CType(dgvResumen.Rows(e.RowIndex).DataBoundItem, clsInfoExportarMercadoPago)
        If auxInfo.EstadoContrato > 0 Then
          CType(dgvResumen.Rows(e.RowIndex).DataBoundItem, clsInfoExportarMercadoPago).Exportar = False
        Else
          CType(dgvResumen.Rows(e.RowIndex).DataBoundItem, clsInfoExportarMercadoPago).Exportar = Not CType(dgvResumen.Rows(e.RowIndex).DataBoundItem, clsInfoExportarMercadoPago).Exportar
        End If
        ClsInfoExportarMercadoPagoBindingSource.ResetBindings(False)
        dgvResumen.Refresh()
        lblResumen.Text = String.Format("Total de registros: {0}/{1}", m_Banco.countRegistrosAExportar, m_Banco.countTotalRegistros)
  ...
  Private Sub dgvResumen_ColumnHeaderMouseClick(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvResumen.ColumnHeaderMouseClick
    Try
      ...
      If m_CurrentSortColumn.HeaderCell.SortGlyphDirection = SortOrder.Descending Or m_CurrentSortColumn.HeaderCell.SortGlyphDirection = SortOrder.None Then
        ...
        Next
        ClsInfoExportarMercadoPagoBindingSource.DataSource = m_Banco.m_RegistrosExportar.OrderBy(Function(c) CStr(c.GetType.GetProperty(m_CurrentSortColumn.DataPropertyName).GetValue(c)), New clsComparar).ToList()
      ...
      Else
        ...
        ClsInfoExportarMercadoPagoBindingSource.DataSource = m_Banco.m_RegistrosExportar.OrderByDescending(Function(c) CStr(c.GetType.GetProperty(m_CurrentSortColumn.DataPropertyName).GetValue(c)), New clsComparar).ToList()
  ...
  Private Sub Refresh_Selection(ByVal indice As Integer)
    Try
      ...
      If (indice >= 0) Then
        Dim auxInfo As New clsInfoExportarMercadoPago
        auxInfo = CType(dgvResumen.Rows(indice).DataBoundItem, clsInfoExportarMercadoPago)
  ...
  Private Sub cmbEstado_SelectionChangeCommitted(sender As Object, e As EventArgs) Handles cmbEstado.SelectionChangeCommitted
    Try
      ...
      If (dgvResumen.SelectedRows(0).Index >= 0) Then
        Dim auxInfo As New clsInfoExportarMercadoPago
        auxInfo = CType(dgvResumen.Rows(dgvResumen.SelectedRows(0).Index).DataBoundItem, clsInfoExportarMercadoPago)
