...
Public Class clsListAdelantos
  Inherits clsList(Of clsInfoAdelanto)
  ...
  Protected Overrides Function RefreshData_Private() As libCommon.Comunes.Result
    Try
      ...
      Try
        ...
        Dim objListArticuloInfo As clsInfoAdelanto
        Dim strCommand As String = "Adelantos" ', (select max(bcospac.fecha) as LastVisitDate from BcosPac where BcosPac.idPac = Pac.idPac) as  LastVisitDate from Pac)"
        ...
        If auxResult > 0 Then
          For Each fila As DataRow In objDatos.Table.Rows
            objListArticuloInfo = New clsInfoAdelanto
            objResult = clsAdelantos.AdelantoIgualDataRow(objListArticuloInfo, fila)
            If objResult <> Result.OK Then Return objResult
