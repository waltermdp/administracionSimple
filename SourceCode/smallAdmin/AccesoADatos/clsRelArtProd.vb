...
Public Class clsRelArtProd
  ...
  Public Shared Function Init(ByVal vObjDB As libDB.clsAcceso, ByRef rlistArticulos As List(Of clsInfoArticulos), ByVal vGuidProducto As Guid, Optional ByRef rCodeError As Integer = -1) As Result
    Try
      ...
      strSQL = "SELECT * FROM [Articulos] INNER JOIN [RelArtProd] ON Articulo.GuidArticulo = RelArtProd.GuidArticulo WHERE [RelArtProd.GuidProducto]={" & vGuidProducto.ToString & "}"
      ...
      For Each dr As DataRow In dt.Rows
        ...
        objResult = clsRelArtProd.ArticuloIgualDataRow(auxInfoArticulos, dr)
  ...
  Public Shared Function Save(ByVal vObjDB As libDB.clsAcceso, ByRef rListArticulos As List(Of clsInfoArticulos)) As Result
    Try
      Dim objResult As Result = Result.OK
      For Each pago In rListArticulos
        objResult = SaveArti(vObjDB, pago)
        If objResult <> Result.OK Then Exit For
      Next
      Return objResult
    Catch ex As Exception
  ...
  Public Shared Function ArticuloIgualDataRow(ByRef vInfoArticulo As clsInfoArticulos, ByVal vData As DataRow) As Result
    Try
      With vData
        Try
          vInfoArticulo.IdPago = CInt(IIf(IsDBNull(.Item("IdPago")), -1, .Item("IdPago")))
        Catch ex As Exception
          vInfoArticulo.IdPago = -1
          Call Print_msg(ex.Message)
        End Try

        Try
          vInfoArticulo.GuidPago = CType(IIf(IsDBNull(.Item("GuidPago")), Nothing, .Item("GuidPago")), Guid)
        Catch ex As Exception
          vInfoArticulo.GuidPago = Nothing
          Call Print_msg(ex.Message)
        End Try
